# WhatLoll-Encoder
Шифровка/Расшифровка на шифре WhatLoll Encoder

<div align="center">
  <img src="https://img.shields.io/badge/version-2.0-blue.svg" alt="Version 2.0"/>
  <img src="https://img.shields.io/badge/license-MIT-green.svg" alt="MIT License"/>
  <img src="https://img.shields.io/badge/platform-Windows%20Forms-blueviolet" alt="Platform"/>
  <img src="https://img.shields.io/badge/.NET-4.6.2%2B-orange" alt=".NET"/>
  <img src="https://img.shields.io/badge/UI-WinForms-lightgrey" alt="UI"/>
</div>

<div align="center">
  <h3>🎮 Простая программа для шифрования текста</h3>
  <p>Шифруйте и расшифровывайте текст с помощью уникальных таблиц замен WhatLoll</p>
</div>

---

## 📥 Скачать приложение

<div align="center">

| Версия | Скачать | Размер | Требования |
|--------|---------|--------|------------|
| **Latest (v2.0)** | [![Download](https://img.shields.io/badge/Download-WhatLoll.Encoder.v2.0-green)](https://github.com/xachapyri-dev/WhatLoll-Encoder/releases/download/v2.0/WhatLoll.Encoder.v2.0.zip) | ~150 KB | Windows 7+ / .NET 4.6.2+ |
| **Source Code** | [![Source](https://img.shields.io/badge/Source-GitHub-black)](https://github.com/yourusername/WhatLoll-Encoder) | - | Visual Studio 2022 |

</div>

### ⚡ Быстрая установка

```bash
# 1. Скачайте
# 3. Запустите WhatLollEncoder.exe
# 4. Готово! 🎉
```

> [!WARNING]
> Данный шифр не подходит для шифровки важных данных! Расшифровка на 100%

# Библиотеки

<div align="center">

| Тип | Язык | Открытый код | Скачать | Скачать по команде |
|-----|------|--------------|---------|--------------------|
| DLL | C# | [Name](url) | [Name](url) | - |
| JS | Java Script | [Name](url) | [Name](url) | - |
| PY | Python | [Name](url) | [Name](url) | - |

</div>
